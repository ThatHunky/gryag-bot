...
class DatabaseService {
  ...
  async saveMessage(messageData) {
    return new Promise((resolve, reject) => {
      ...
      const embeddingBlob = embedding ? Buffer.from(JSON.stringify(embedding)) : null;
      ...
      this.db.run(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        chatId, userId, firstName, lastName, username, messageText,
        messageType, mediaCaption, embeddingBlob, timestamp, replyToMessageId
      ], function(err) {
        if (err) {
  ...
  async getRecentMessages(chatId, limit = 20, maxDays = 30) {
    return new Promise((resolve, reject) => {
      ...
      this.db.all(`
        ...
        LIMIT ?
      `, [chatId, maxTimestamp, limit], (err, rows) => {
        if (err) {
  ...
  async updateMessageEmbedding(messageId, embedding) {
    return new Promise((resolve, reject) => {
      ...
      this.db.run(`
        ...
        SET embedding = ?
        WHERE id = ?
      `, [embeddingBlob, messageId], function(err) {
        if (err) {
